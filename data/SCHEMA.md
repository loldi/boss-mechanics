...
| Field | Type | Notes |
|---|---|---|
| `animationId` | int? | Animation played on the boss model in the viewer. **Ignored when `staticFallback` is true** — a static pose always wins, even if this is also set. Required when `staticFallback` is false (or absent) and no `animationChain` is curated; the loader rejects a mechanic with none of the three. **Mutually exclusive with `animationChain`** — curate one or the other, never both. |
| `animationChain` | object[]? | Ordered animation segments played back to back, looping as a whole once the last one ends (docs/DECISIONS.md D37), e.g. Doom's Shockwave: charge → charge loop → slam, instead of holding on the slam alone. Each entry is `{animationId, cycles}` — an object per segment, not two parallel lists, so an id and its curated duration are one fact together. `cycles` is the number of **client cycles (20ms each)** to show that segment before advancing: for a classic (non-skeletal) sequence this is the sum of `frameLengths` across the sequence; for a skeletal sequence (docs/DECISIONS.md D36) there is no `frameLengths` to sum, so it is `animMayaEnd` from the cachetool's `DumpSeqMaya` (outside this repo, per D36's split). Needs at least 2 segments — one segment is just `animationId` spelled a longer way, so the loader rejects it. **Ignored when `staticFallback` is true**, the same as `animationId`. When present, `animationId` must be absent; the ordinary single-animation path resolves to the chain's own first segment instead. **First pass exact, later passes may resume mid-animation**: each segment plays on the same D24 pool-widget-per-animation-id the rest of the preview system uses, so a segment whose widget was already created earlier in the chain (most commonly segment 0, on the second and later loops) reuses that widget rather than recreating it — and reusing a pool widget resumes wherever its own frame counter currently sits (D24's existing resume-not-restart behaviour), not necessarily frame 0. This is invisible at 20ms in practice and is not something to compensate for; see docs/DECISIONS.md D37. |
...
validator error. **The ladder is unchanged by `animationChain` (docs/DECISIONS.md D37): `sprite`
still wins over a curated chain, and `staticFallback` still wins over one too, the same as either
already wins over a plain `animationId`** — a chain only ever substitutes for `animationId` itself,
one level of the same ladder. **The preview npc need not be the trigger npc** — curate whichever variant actually
